...
class DatabaseService {
  ...
  async listUpcomingSlots({ dateFilter = null, locationFilter = '' } = {}) {
    try {
      if (!this.client) {
        ...
        if (slots.length === 0) {
          // Utiliser des dates futures pour les tests
          const today = new Date();
          const day5 = new Date(today.getFullYear(), today.getMonth(), today.getDate() + 2);
          const day10 = new Date(today.getFullYear(), today.getMonth(), today.getDate() + 7);
          const day15 = new Date(today.getFullYear(), today.getMonth(), today.getDate() + 12);
          
          slots = [
            {
              slot_id: 'mock_1',
              start_at: new Date(day5.setHours(9, 0, 0, 0)).toISOString(),
              location: 'Place de la Mairie',
              date: day5.toISOString().slice(0, 10)
            },
            {
              slot_id: 'mock_2',
              start_at: new Date(day5.setHours(9, 15, 0, 0)).toISOString(),
              location: 'Place de la Mairie',
              date: day5.toISOString().slice(0, 10)
            },
            {
              slot_id: 'mock_3',
              start_at: new Date(day10.setHours(14, 0, 0, 0)).toISOString(),
              location: 'Rue de la Station',
              date: day10.toISOString().slice(0, 10)
            },
            {
              slot_id: 'mock_4',
              start_at: new Date(day15.setHours(10, 0, 0, 0)).toISOString(),
              location: 'Place de l\'Église',
              date: day15.toISOString().slice(0, 10)
            }
          ];
        }
  ...
  async getSlotById(slotId) {
    try {
      if (!this.client) {
        // Search in memory store when Supabase is not available
